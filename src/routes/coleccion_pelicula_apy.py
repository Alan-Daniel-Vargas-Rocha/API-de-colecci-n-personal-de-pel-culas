...
@router.put("/{coleccion_id}", response_model = ColeccionPeliculaResponseDTO, status_code = status.HTTP_202_ACCEPTED)
def update_coleccion_pelicula(coleccion_id: int, data: ColeccionPeliculaUpdateDTO, db: Session = Depends(get_db)):
    return ColeccionPeliculaService.update_coleccion_pelicula(coleccion_id = coleccion_id, dto = data, db = db)

@router.delete("/{coleccion_id}", status_code = status.HTTP_204_NO_CONTENT)
def delete_coleccion_pelicula(coleccion_id: int, db: Session = Depends(get_db)):
    ColeccionPeliculaService.delete_coleccion_pelicula(coleccion_id = coleccion_id, db = db)
